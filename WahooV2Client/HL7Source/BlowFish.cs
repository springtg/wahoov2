    ...
    public class BlowFish
    {
        ...
        public void InitBoxes(byte[] key)
        {
            ...
            for (i = 0; i < N + 2; ++i)
            {
                ...
                //DEBUG_LOG.Write("Output Data Start ***********************");
                for (k = 0; k < 4; ++k)
                {
                    data = (data << 8) | Key[j];
                    //DEBUG_LOG.Write(data);
                    j = j + 1;
                    if (j >= Key.Length)
                    {
                        j = 0;
            ...
            //DEBUG_LOG.Write("initialize bl: " + bl + " br: " + br);

            for (i = 0; i < N + 2; i += 2)
            {
                Encipher(ref bl, ref br);
                P[i] = bl; P[i + 1] = br;
                //DEBUG_LOG.Write("TEST1. P[i]: " + P[i] + " P[i + 1]: " + P[i + 1]);
            }

            //DEBUG_LOG.Write("initialize 2 bl: " + bl + " br: " + br);

            for (int row = 0; row < 4; row++)
            {
                //DEBUG_LOG.Write("Start For 2.1");
                for (int col = 0; col < 256; col += 2)
                {
                    //DEBUG_LOG.Write("Start Encipher 2: Position " + col);
                    Encipher(ref bl, ref br);
                    S[row, col] = bl; S[row, col + 1] = br;
                    //DEBUG_LOG.Write("TEST2. S[row, col]: " + bl + " S[row, col + 1]: " + br);
                }
            }
        }
        ...
        private long F(long x)
        {
            ...
            //.. x(32bits) => (a, b, c, d)
            d = (int)(x & 0x00FF);
            //DEBUG_LOG.Write("1. x: " + x);
            x >>= 8;
            //DEBUG_LOG.Write("1. x >>= 8: " + x);
            c = (int)(x & 0x00FF);
            //DEBUG_LOG.Write("2. x: " + x);
            x >>= 8;
            //DEBUG_LOG.Write("2. x >>= 8: " + x);
            b = (int)(x & 0x00FF);
            //DEBUG_LOG.Write("3. x: " + x);
            x >>= 8;
            //DEBUG_LOG.Write("3. x >>= 8: " + x);
            a = (int)(x & 0x00FF);
            //DEBUG_LOG.Write("a, b, c, d: " + a + ", " + b + ", " + c + ", " + d);
            long y = 0;

            //.. y = (S[0,a] + S[1, b] mod 2**32) XOR S[2, c] + S[3, d] mod 2**32.
            y += (long)S[0, a] + (long)S[1, b];
            //DEBUG_LOG.Write("y1: " + y);
            y ^= S[2, c];
            //DEBUG_LOG.Write("y2: " + y);
            y += S[3, d];
            //DEBUG_LOG.Write("y3: " + y);
            //y &= 0xFFFFFFFF;
            y &= 4561237890L;
            //DEBUG_LOG.Write("y4: " + y);
        ...
        /// <param name="xl"></param>
        /// <param name="xr"></param>
        private void Encipher(ref long xl, ref long xr)
        {
            long Xl;
            long Xr;
            long temp;
            ...
            //DEBUG_LOG.Write("Perform Feistel cycle 16 times. ****************");
            for (i = 0; i < N; ++i)
            {
                //DEBUG_LOG.Write("");
                //DEBUG_LOG.Write("Xl ^ P[i]:" + Xl + " ^ " + P[i]);
                ////DEBUG_LOG.Write("F(Xl) ^ Xr:" + F(Xl) + "^" + Xr);
                Xl = Xl ^ P[i];
                Xr = F(Xl) ^ Xr;
                //DEBUG_LOG.Write("Xl: " + Xl + ", Xr: " + Xr);
        ...
        /// <param name="xl"></param>
        /// <param name="xr"></param>
        private void Decipher(ref long xl, ref long xr)
        {
            long Xl;
            long Xr;
            long temp;
        ...
        public byte[] Encipher(byte[] idata)
        {
            ...
            {
                xl = ((long)odata[i] << 24) | ((long)odata[i + 1] << 16) | ((long)odata[i + 2] << 8) | (long)odata[i + 3];
                xr = ((long)odata[i + 4] << 24) | ((long)odata[i + 5] << 16) | ((long)odata[i + 6] << 8) | (long)odata[i + 7];
        ...
        {
            //if (idata.Length < 8) return idata;
            //DEBUG_LOG.Write("Start Decipher Buffer ************");
            int length = (idata.Length / 8) * 8;
            byte[] odata = idata.Clone() as byte[];
            ...
            for (int i = 0; i < length; i += 8)
            {
                //DEBUG_LOG.Write("Before xl: " + xl + ", xr: " + xr);
                xl = ((long)odata[i] << 24) | ((long)odata[i + 1] << 16) | ((long)odata[i + 2] << 8) | (long)odata[i + 3];
                xr = ((long)odata[i + 4] << 24) | ((long)odata[i + 5] << 16) | ((long)odata[i + 6] << 8) | (long)odata[i + 7];
                //DEBUG_LOG.Write("Before oxl: " + xl + ", oxr: " + xr);
            ...
            {
                //DEBUG_LOG.Write("Result buffer[" + j + "]: " + odata[j]);
            }
        ...
        public bool Decipher(string fin, string fout)
        {
            ...
            try
            {
                ...
                {
                    buff = rin.ReadBytes(block);
                    //DEBUG_LOG.Write("No of Bytes: " + buff.Length);
                    for (int i = 0; i < buff.Length; i++)
                    {
                        //DEBUG_LOG.Write("Buffer[" + i + "]: " + buff[i]);
                    }
